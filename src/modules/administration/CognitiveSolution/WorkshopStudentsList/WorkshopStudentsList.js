...
const WorkshopStudentsList = (props) => {
  // console.log(internshipId);

  const history = useHistory();
  const [workshopStudentsList, setWorkshopStudentsList] = useState([]);
  ...
  const [workshopId, setWokshopId] = useState(props.match.params.workshopId);
  const [studentRegisterNo, setStudentRegisterNo] = useState();

  let totalActualFees = 0,
    totalPaidFees = 0,
    totalBalance = 0;
  totalActualFees += workshopStudentsList.reduce((sum, obj) => {
    return sum + obj.actual_fees;
  }, 0);
  totalPaidFees += workshopStudentsList.reduce((sum, obj) => {
    return sum + obj.paid;
  }, 0);
  totalBalance += workshopStudentsList.reduce((sum, obj) => {
  ...
  const fetchData = () => {
    ...
    axios
      .get(`accounts/all/fees/workshop_students/?workshop=${workshopId}`, headers)
      .then((res) => {
        console.log(res.data);
        setWorkshopStudentsList(res.data);
      })
      .catch((err) => console.log(err));

    axios
      .get(
        `administration/cognitive_solutions/programs/workshops/${workshopId}`,
        headers
      )
      .then((res) => {
        console.log(res.data);
        setWorkshopName(res.data.name);
      })
      .catch((err) => console.log(err));
  ...
  const paymentSubmitHandler = (event) => {
    ...
    console.log(data);
    if(studentRegisterNo.substring(0, 2) === 'AW'){
      axios
      .post("accounts/all/fees/academic_workshop_fees/", data, headers)
      .then((res) => {
        console.log(res.data);
    ...
    if(studentRegisterNo.substring(0, 2) === 'CW'){
      axios
        ...
        <Col md="12">
          <Card>
            <CardHeader className="table__header">
              <CardTitle tag="h4">Students of {workshopName}</CardTitle>
            ...
            <CardBody>
              <h6>
                Number of students: {workshopStudentsList.length} &nbsp; Total
              ...
              <Table>
                ...
                <tbody>
                  {workshopStudentsList.map((listItem) => {
                    return (
                      <tr
                        className="tableRow"
                        // onClick={() => {
...
export default WorkshopStudentsList;
